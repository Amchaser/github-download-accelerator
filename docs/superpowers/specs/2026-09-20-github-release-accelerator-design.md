...
- **目标文件在 `close()` 之前一直是 0 字节**（2026-09-20 实测确认：一次跑到 98.5% 的
  运行，磁盘上目标文件仍是 0 字节）。FSA 的写入先进**临时文件**，只有 `close()` 才原子
  替换到目标路径。后果是**在上限处失败 = 全部作废**——磁盘上没有任何可续传的半成品，
  重试只能从头再来。这比「关掉标签页就得重下」更严重：它把「接近完成」与「刚开始」
  变成了等价状态，且用户从文件系统看不出任何进展。
- **实测吞吐（2026-09-20）**：4 连接 **11.8–12.9 MiB/s**（492 MB 用时约 41 秒）；
  单连接经 `gh.xmly.dev` 达 **6.88 MB/s**（499558899 字节用时 72.6 秒）。
  内存方面：JS 堆峰值 77 MiB，浏览器进程峰值约 150 MB，且下载结束后回落——
  与「4 个在途分块 × 8 MiB」的量级吻合，**不随文件大小线性增长**。
...
4. 块大小已降到下限（1 MB）仍失败 → **硬失败**：拆分返回 null，随后尝试预算耗尽即抛错。
   （**没有**「转为单连接顺序下载剩余区间」这一步——`fallbackDownload` 只从 `acceptRanges
   === false` 分支进入，与本降级路径无关。原表述描述的机制从未实现，2026-09-20 终审修正。）
...
4. **写入必须被 `await`，且携带正确的绝对 `position`**。不同 worker 的写入**可以**并发在途——
   FSA 在内部对写入排队串行化，而显式 `position` 使写入顺序无关。**此点经实测确认**：
   16 分块 / 4 worker 并发定位写入，产出文件 SHA-256 与参照逐字节一致。
   （初稿曾写「必须串行化，并发写无意义且危险」，与上述实测矛盾，已按证据修正。）
